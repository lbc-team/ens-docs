...
为了防止域名抢注，.eth注册器管理员需要对新域名注册(但不需要对续期)执行一个“申请-揭示”的过程。要注册一个域名，用户必须:
...
这个过程保证了注册不会被抢注，除非攻击者能够审查至少1分钟前用户申请注册域名的交易。
...
这个公共常量表示申请的最短时间，以秒为单位。一次申请只能在它被打包后至少经过这么多秒才能被揭示。
