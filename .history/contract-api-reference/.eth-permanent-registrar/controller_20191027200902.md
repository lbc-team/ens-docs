...
为了防止域名抢注，.eth注册器管理员需要对新域名注册(但不需要对续期)执行一个“提交-揭示”的过程。要注册一个域名，用户必须:
...
这个过程保证了注册不会被抢注，除非攻击者能够审查至少1分钟前用户提交待注册域名的交易。
...
这个公共常量表示提交的最短时间，以秒为单位。一次提交只能在它被打包后至少这么多秒之后才能被揭示。
...
DApps应该获取这个常量，而不是对当前值进行硬编码，因为它可能会在以后的升级中发生变化。
