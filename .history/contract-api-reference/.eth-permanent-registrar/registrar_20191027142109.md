...
如果域名可以注册，则返回`true`。考虑到旧版注册器上那些尚未迁移的已注册域名，注册器的控制器可能会对注册施加比此合约更多的限制（例如，最小域名长度），因此此函数**不应该**用于检查用户是否可以注册某个域名。要检查用户是否可以注册一个域名，[通过控制器检查名称的可用性](controller.md#check-name-availability)。
