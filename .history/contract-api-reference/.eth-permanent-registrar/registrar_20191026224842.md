...
注册器只处理标签散列——标签的第一个组件的`keccak256`（例如，`keccak256('ens')``ens.eth`）。为了与ERC721兼容，这些值表示为uint256值，而不是bytes32值，但是可以透明地进行双向转换。名称的namehash可以通过计算“keccak256(baseNode, labelHash)”得到，其中“baseNode”是注册机构管理的TLD的namehash—例如，“namehash('eth')”。
