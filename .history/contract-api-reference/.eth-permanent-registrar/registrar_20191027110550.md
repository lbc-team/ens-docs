...
直接在.eth下分配的域名（例如，以.eth结尾的二级域名，如 _alice.eth_ ）是由这里所说.eth永久注册器管理的。当从注册器购买一个域名并合法获取该域名在ENS中的所有权时，注册器会自己跟踪和记录谁拥有**注册**，即谁是注册人，这里的**注册人**的对.eth永久注册器来说是唯一的。（译注：这里提到的“注册”是个名词，可以理解为获得一个域名所有权的事实，拥有一个域名的“注册”的人，就是这个域名的“注册人”，其实就是这个域名的所有者，不知道ENS团队为啥把这几个概念弄得这么复杂。）
...
域名的注册人可以将注册转移到另一个帐户，他们可以通过调用[reclaim](registrar.md#reclaim-ens-record)来恢复域名的所有权，该方法将ENS域名的所有权重置到注册人的帐户。
...
将域名的所有权和注册的所有权分离可以更容易地构建对ENS进行自动更新的系统，拥有注册可以更容易地构建系统,自动更新ENS.注册人可以将名称的所有权转移到另一个帐户或一个聪明的合同管理记录、子域,等,同时仍然保留为升级,恢复能力的所有权或妥协的情况下。
