...
注册器只处理标签散列，确切地说是标签的第一个组件的`keccak256`值（比如，对`ens.eth`来说就是`keccak256('ens')`）。为了与ERC721兼容，这些值表示为uint256值，而非bytes32值，但是可以相互透明转换。域名的namehash可以通过计算`keccak256(baseNode, labelHash)`获得，其中`basenode`是注册器所管理的TLD的namehash，比如`namehash('eth')`。
