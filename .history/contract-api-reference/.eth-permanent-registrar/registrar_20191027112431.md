...
直接在.eth下分配的域名（例如，以.eth结尾的二级域名，如 _alice.eth_ ）是由这里所说.eth永久注册器管理的。当从注册器购买一个域名并合法获取该域名在ENS中的所有权时，注册器会自己跟踪谁拥有**注册**，即谁是注册人，这里的**注册人**的对.eth永久注册器来说是唯一的。（译注：这里提到的“注册”是个名词，可以理解为“注册权”，注册权是一个域名的最高操作权限，拥有一个域名的注册权，就是这个域名的“注册人”，注册人可以操作所有权。）
