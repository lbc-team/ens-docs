...
直接在.eth下分配的域名（例如，以.eth结尾的二级域名，如 _alice.eth_ ）是由这里所说.eth永久注册器管理的。当从注册器购买一个域名并合法获取该域名在ENS中的所有权时，注册器会自己跟踪和记录谁拥有**注册**，即谁是注册人，这里的**注册人**的对.eth永久注册器来说是唯一的。（译注：这里提到的“注册”是个名词，可以理解为获得一个域名所有权的事实，拥有一个域名的“注册”的人，就是这个域名的“注册人”，其实就是这个域名的所有者。）
