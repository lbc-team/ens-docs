...
如果域名可以注册，则返回`true`。考虑到旧版注册中心上那些尚未迁移的已注册域名，注册中心的控制器可能会对域名注册增加一些超出注册中心合约的限制条件（例如，最小域名长度），所以该函数**不应该**用于检查用户是否可以注册某个域名。要检查用户是否可以注册某个域名，应该[通过控制器检查域名的可用性](controller.md#check-name-availability)。
